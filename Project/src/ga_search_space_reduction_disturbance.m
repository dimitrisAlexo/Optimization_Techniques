...
c = [54.13 21.56 34.08 33.03 29.96 47.24 26.89 32.76 37.12 49.19 21.84 24.87 33.97 39.98 53.83 61.65 59.73];
V = 100;
...
fprintf('Done!\n');
x = decode(bounds, n_bits, best);

x(10) = -x(1)-x(2)-x(3)+V;
x(11) = x(1)-x(4);
x(12) = x(2)-x(5);
x(13) = -x(1)-x(2)-x(3)-x(6)+V;
x(14) = x(2)+x(3)-x(5)+x(6)-x(7)-x(8);
x(15) = x(1)+x(2)+x(3)-x(4)+x(6)-x(7)-x(8)-x(9);
x(16) = x(4)+x(9);
x(17) = -x(1)-x(2)-x(3)-x(6)+x(7)+V;

%mapping

x_mapped = [x(1) x(2) x(3) x(10) x(4) x(11) x(5) x(12) x(6) x(13) x(7) x(8) x(14) x(9) x(15) x(16) x(17)];
...
for i = [0.85 0.90 0.95 1 1.05 1.10 1.15]
    for j = 1 : 17
        x_mapped(j) = x_mapped(j)*i;
    end
    
    sum = 0;
    for j = 1 : 17
        T = a(j)*x_mapped(j)/(1-x_mapped(j)/c(j));
        sum = sum + x_mapped(j)*T;
    end
    
    fprintf('For V_disturb = %.2fV the best time is: %.2f with an average constraint violation rate of: %.2f%%\n', ...
        i, sum/(100*i), constraint_violation(x_mapped, i*100)*100);
end
...
function violation = constraint_violation(x, V)
    violation = abs(V-x(1)-x(2)-x(3)-x(4)) + abs(x(1)-x(5)-x(6)) + abs(x(2)-x(7)-x(8)) ...
        + abs(x(3)+x(8)+x(9)-x(13)-x(12)-x(11)) + abs(x(4)-x(9)-x(10)) + abs(x(5)+x(14)-x(16)) ...
        + abs(x(12)+x(17)+x(16)+x(15)-V) + abs(x(10)+x(11)-x(17)) + abs(x(6)+x(7)+x(13)-x(14)-x(15));
    violation = violation/(9*V); %mean value
end
